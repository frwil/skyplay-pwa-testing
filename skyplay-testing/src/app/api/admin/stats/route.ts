  ...
  try {
    ...
    const usersRs = await db.execute(
      `SELECT
        ...
        COALESCE(SUM(CASE WHEN s.status = 'APPROVED' THEN q.reward_amount ELSE 0 END), 0)
          ...
          as total_rewards,
        COALESCE(u.participation_bonus, 0) as participation_bonus,
        u.bonus_status,
    ...
    const overviewRs = await db.execute(
      `SELECT
        ...
        COALESCE((SELECT SUM(q.reward_amount) FROM submissions s JOIN questions q ON s.question_id = q.id WHERE s.status = 'APPROVED'), 0)
          + COALESCE((SELECT SUM(participation_bonus) FROM users WHERE bonus_status = 'APPROVED'), 0)
          as total_sky_distributed,
        (SELECT COUNT(*) FROM users WHERE bonus_status = 'PENDING') as pending_bonus_count`
    );
